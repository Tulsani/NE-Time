...
class TimeSeriesDataset(Dataset):
    ...
    def __init__(
        ...
        max_start = len(self.data) - seq_len - self.max_h + 1
        self.indices = list(range(0, max(1, max_start), stride))
...
def create_dataloaders(
    ...
    for ds in [val_ds, test_ds]:
        max_start = len(ds.data) - seq_len - max(horizons) + 1
        ds.indices = list(range(0, max(1, max_start), 1))
